...
class RNNCellBase(nn.Cell):
    def __init__(self, input_size: int, hidden_size: int, bias: bool, num_chunks: int):
        ...
        self.bias = bias
        self.weight_ih = Parameter(Tensor(np.random.uniform(-1/hidden_size, 1/hidden_size, (num_chunks * hidden_size, input_size)).astype(np.float16)))
        self.weight_hh = Parameter(Tensor(np.random.uniform(-1/hidden_size, 1/hidden_size, (num_chunks * hidden_size, hidden_size)).astype(np.float16)))
        if bias:
            self.bias_ih = Parameter(Tensor(np.random.uniform(-1/hidden_size, 1/hidden_size, (num_chunks * hidden_size)).astype(np.float16)))
            self.bias_hh = Parameter(Tensor(np.random.uniform(-1/hidden_size, 1/hidden_size, (num_chunks * hidden_size)).astype(np.float16)))
