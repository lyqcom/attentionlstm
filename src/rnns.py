...
from my_utils import Reverse, ReverseSequence
...
class DynamicRNN(nn.Cell):
    ...
    def recurrent(self, x, h_0, w_ih, w_hh, b_ih, b_hh):
        ...
        h = h_0
        while t < time_step:
            h = self.cell(self.squeeze_0(x[t:t+1]), h, w_ih, w_hh, b_ih, b_hh)
            if self.is_lstm:
                outputs.append(h[0])
            else:
                outputs.append(h)
    ...
    def __init__(self, mode, input_size, hidden_size, num_layers=1, has_bias=True, batch_first=False, dropout=0, bidirectional=False):
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                self.w_ih_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (gate_size, layer_input_size)).astype(np.float16)), name='weight_ih_l{}{}'.format(layer, suffix)))
                self.w_hh_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (gate_size, hidden_size)).astype(np.float16)), name='weight_hh_l{}{}'.format(layer, suffix)))
                if has_bias:
                    self.b_ih_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (gate_size)).astype(np.float16)), name='bias_ih_l{}{}'.format(layer, suffix)))
                    self.b_hh_list.append(Parameter(Tensor(np.random.uniform(-stdv, stdv, (gate_size)).astype(np.float16)), name='bias_hh_l{}{}'.format(layer, suffix)))
